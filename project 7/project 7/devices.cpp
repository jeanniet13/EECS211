...
device::device(char *n, int sn) {  //n is the name, sn is the serial number 
	...
	void strcpy(char* name,char* n);  //copy string n to newly allocated space
...
DigitalSensorDevice::DigitalSensorDevice(char* n, int sn, int v) {
	name = (char *) malloc(strlen(n)+1);  //allocate space to hold string n and assign returned address to device name data member
	strcpy(name, n);  //copy string n to newly allocated space
	serialnumber = sn;  //copy sn to serial number data member
	value = v;
...
AnalogSensorDevice::AnalogSensorDevice(char* n, int sn, int v, int br) {
	name = (char *) malloc(strlen(n)+1);  //allocate space to hold string n and assign returned address to device name data member
	strcpy(name, n);  //copy string n to newly allocated space
	serialnumber = sn;  //copy sn to serial number data member
	type = ANALOG_SENSOR;
...
DigitalControllerDevice::DigitalControllerDevice(char* n, int sn) {
	name = (char *) malloc(strlen(n)+1);  //allocate space to hold string n and assign returned address to device name data member
	strcpy(name, n);  //copy string n to newly allocated space
	serialnumber = sn;  //copy sn to serial number data member
	value = OFF;
...
AnalogControllerDevice::AnalogControllerDevice(char* n, int sn, int v, int ub, int lb) {
	name = (char *) malloc(strlen(n)+1);  //allocate space to hold string n and assign returned address to device name data member
	strcpy(name, n);  //copy string n to newly allocated space
	serialnumber = sn;  //copy sn to serial number data member
	lowerbound = lb;
